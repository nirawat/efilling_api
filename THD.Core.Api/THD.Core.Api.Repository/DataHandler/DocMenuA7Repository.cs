...
namespace THD.Core.Api.Repository.DataHandler
{
    public class DocMenuA7Repository : IDocMenuA7Repository
    {
        ...
        public async Task<ModelResponseA7Message> AddDocMenuA7Async(ModelMenuA7 model)
        {
            ...
            ModelResponseA7Message resp = new ModelResponseA7Message();
        ...
        public async Task<ModelMenuA7_InterfaceData> MenuA7EditInterfaceDataAsync(string UserId, string ProjectNumber)
        {
            ...
            resp.editdata = await GetMenuA7DataEditAsync(ProjectNumber, UserId, resp.UserPermission);
        ...
        private async Task<ModelMenuA7> GetMenuA7DataEditAsync(string ProjectNumber, string userid, ModelPermissionPage permission)
        {
        ...
        public async Task<ModelMenuA7_FileDownload> GetA7DownloadFileByIdAsync(int DocId, int Id)
        {
            ...
            string sql = "SELECT TOP(1) file1name,file2name FROM Doc_MenuA7 WHERE doc_id='" + DocId + "' ";
            ...
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    ...
                    if (reader.HasRows)
                    {
                        ModelMenuA7_FileDownload e = new ModelMenuA7_FileDownload();
                        while (await reader.ReadAsync())
                        {
                            if (Id == 1)
                            {
                                e.filebase64 = ServerDirectorys.ReadFileToBase64(_IEnvironmentConfig.PathDocument, FolderDocument.menuA7, reader["file1name"].ToString());
                                e.filename = "รายงานผลสรุปและแจ้งปิดโครงการ";
                            }
                            if (Id == 2)
                            {
                                e.filebase64 = ServerDirectorys.ReadFileToBase64(_IEnvironmentConfig.PathDocument, FolderDocument.menuA7, reader["file2name"].ToString());
                                e.filename = "ไฟล์ผลงานวิจัยฉบับสมบูรณ์";
                            }
                        }
                        return e;
                    }
        ...
        public async Task<ModelResponseA7Message> UpdateDocMenuA7EditAsync(ModelMenuA7 model)
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("sp_doc_menu_a7_edit", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    if ((int)cmd.Parameters["@rStatus"].Value > 0)
                    {
                        ...
                        model_rpt_5_file rpt = await _IDocMenuReportRepository.GetReportR5Async(Convert.ToInt32(model.docid));
