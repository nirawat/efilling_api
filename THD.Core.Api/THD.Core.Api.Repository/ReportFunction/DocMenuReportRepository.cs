...
namespace THD.Core.Api.Repository.DataHandler
{
    public class DocMenuReportRepository : IDocMenuReportRepository
    {
        ...
        public async Task<model_rpt_1_file> GetReportR1_2Async(int doc_id)
        {
            ...
            try
            {
                model_rpt_1_report rptData = new model_rpt_1_report();
                ...
                string report_name = "A1_" + doc_id + DateTime.Now.ToString("_yyyyMMddHHmmss").ToString() + ".pdf";
                string report_full_path = _IEnvironmentConfig.PathReport + report_name;

                if (rptData != null && rptData.projecttype == "1")
                {
                    rpt1.DataSource = dataSource;
                    rpt1.ExportToPdf(report_full_path);
                }

                if (rptData != null && rptData.projecttype == "2")
                {
                    rpt2.DataSource = dataSource;
                    rpt2.ExportToPdf(report_full_path);
                }
        ...
        public async Task<model_rpt_10_file> GetReportR10Async(int doc_id)
        {
            ...
            try
            {
                ...
                string report_name = "?_" + doc_id + DateTime.Now.ToString("_yyyyMMddHHmmss").ToString() + ".pdf";
                string report_full_path = _IEnvironmentConfig.PathReport + report_name;
        ...
        public async Task<ModelResponseMessage> CloseDocumentMeetingOfYearAsync(int doc_id)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(ConnectionString))
                {
                    ...
                    string round_of_meeting = "";
                    string year_of_meeting = "";

                    string sql = "SELECT meeting_round, year_of_meeting FROM Doc_MenuC3 WHERE doc_id='" + doc_id + "'";

                    using (SqlCommand cmd_of_year = new SqlCommand(sql, conn))
                    {
                        SqlDataReader reader_of_year = await cmd_of_year.ExecuteReaderAsync();

                        if (reader_of_year.HasRows)
                        {
                            while (await reader_of_year.ReadAsync())
                            {
                                round_of_meeting += reader_of_year["meeting_round"].ToString();
                                year_of_meeting += reader_of_year["year_of_meeting"].ToString();
                            }
                        }
                        reader_of_year.Close();
                    }
                    ...
                    string sql_project = "SELECT project_number FROM Doc_MenuC1 WHERE round_of_meeting='" + round_of_meeting + "' AND year_of_meeting='" + year_of_meeting + "'";

                    using (SqlCommand cmd_project = new SqlCommand(sql_project, conn))
                    {
                        SqlDataReader reader_project = await cmd_project.ExecuteReaderAsync();

                        if (reader_project.HasRows)
                        {
                            while (await reader_project.ReadAsync())
                            {
                                list_project += reader_project["project_number"].ToString() + ",";
                            }
                        }
                        reader_project.Close();
                    }

                    string list_project_number = "'" + list_project.Remove(list_project.Length - 1, 1).Replace(",", "','") + "'";


                    using (SqlCommand cmd = new SqlCommand("sp_doc_menu_c3_closed", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add("@doc_id", SqlDbType.Int).Value = doc_id;
                        cmd.Parameters.Add("@round_of_meeting", SqlDbType.Int).Value = round_of_meeting;
                        cmd.Parameters.Add("@year_of_meeting", SqlDbType.Int).Value = year_of_meeting;
                        cmd.Parameters.Add("@project_list", SqlDbType.VarChar, 500).Value = list_project_number;
                        ...
                        if ((int)cmd.Parameters["@rStatus"].Value > 0) resp.Status = true;
        ...
        {
            model_rpt_16_file resp = new model_rpt_16_file();
